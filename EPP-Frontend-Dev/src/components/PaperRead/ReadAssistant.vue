<template>
  <el-container style="height: calc(100vh - 120px);" class="read-assistant">
...
export default {
  ...
  methods: {
    ...
    createPaperStudy () {
      ...
      request.post('/study/createPaperStudy', { 'paper_id': this.paperID, 'file_type': 2 })
        .then((response) => {
          if (response.status === 200) {
            ...
            try {
              this.chatMessages.push({ sender: 'ai', text: response.data['conversation_history']['conversation'][0]['content'], loading: false })
            } catch (error) {
              this.chatMessages.push({ sender: 'ai', text: '知识库构建完成，请向我提问！', loading: false })
            }
    ...
    restorePaperStudy () {
      ...
      request.post('/study/restorePaperStudy', { 'file_reading_id': this.fileReadingID })
        .then((response) => {
    ...
    async chatToAI () {
      ...
      try {
        await request.post('/study/doPaperStudy', { 'query': chatMessage, 'file_reading_id': this.fileReadingID })
          .then(response => {
    ...
    async regenerateAnswer () {
      ...
      await request.post('/study/reDoPaperStudy', { 'file_reading_id': this.fileReadingID })
        .then((response) => {
          answer = response.data.ai_reply
          this.probQuestions = response.data.prob_question
          lastMessage.text = ''
    ...
    renderMarkdown () {
      request.post('/study/generateAbstractReport', { document_id: '', paper_id: this.paperID })
        .then((response) => {
    ...
    clearHistory () {
      request.post('/study/clearConversation', {file_reading_id: this.fileReadingID})
        .then((response) => {
          if (response.status === 200) {
            this.$message({
              message: '清除历史对话成功！',
