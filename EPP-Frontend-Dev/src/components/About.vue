  ...
  <div class="fullscreen-container">
    <div class="carousel-container">
      <el-carousel trigger="click" height="600px">
        <el-carousel-item v-for="member in members" :key="member.id">
          <div class="member-item">
            <div class="member-photo">
              <img :src="member.image" alt="profile" class="img-container">
            </div>
            <div class="member-info">
                <h3>{{ member.name }}<small v-if="member.oldMan" style="font-size: 0.8em;">（前辈）</small></h3>
              <p>{{ member.role }}</p>
              <p><i>{{ member.description }}</i></p>
              <el-button type="text" @click="showModal = true" v-if="member.getMoneyPic">点我打赏</el-button>
            </div>
          </div>
...
.img-container {
  width: 100%;  /* 图像尝试填满容器宽度 */
  height: 100%; /* 图像尝试填满容器高度 */
