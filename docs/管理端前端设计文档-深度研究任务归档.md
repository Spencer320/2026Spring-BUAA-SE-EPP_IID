...
| 类型 | 数量 | 说明 |
|------|------|------|
| 菜单调整 | 1 | 归档入口并入 `DR 管理` 模块顶部子菜单 |
| 路由调整 | 2 | 新增 `/deep-research/archive`；保留 `/deep-research-archive` 兼容跳转 |
...
```text
/deep-research/archive                     ← 左侧菜单「DR 管理」子菜单直达
└── DRTaskList.vue（复用）
    ├── 固定状态标签：状态=已归档
...
在 `DRMonitorManage.vue` 的横向菜单中新增「任务归档」：
