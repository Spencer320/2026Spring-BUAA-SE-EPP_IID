...
class SequencePoolingLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        mask = torch.arange(sequence_embeds.size(1)).expand_as(
            sequence_length
...
class YouTubeNet(nn.Module):
    def __init__(
        ...
        self.sequence_pooling_layer = SequencePoolingLayer(
            sequence_mask_length=sparse_seq_input_length
        )
        ...
        self.item_fc_layers = nn.Linear(
            embedding_dim * 2 * neg_sample_num, neg_sample_num
        )
    ...
    def forward(
        ...
        user_click_item_seq_embedding = self.sequence_pooling_layer(
            [user_click_item_seq_embedding, user_click_item_seq_length]
        )
        ...
        item_embedding = torch.cat(
            [pos_item_sample_embedding, neg_item_sample_embedding], dim=1
        )
