...
class User(models.Model):
    ...
    collected_papers = models.ManyToManyField(
        Paper, related_name="collected_by_users", blank=True
    )
    liked_papers = models.ManyToManyField(
        Paper, related_name="liked_by_users", blank=True
    )
    ...
    def simply_desc(self):
        return {"user_id": str(self.user_id), "user_name": str(self.username)}
