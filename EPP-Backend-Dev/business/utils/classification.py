...
def embed_for_subclass(texts):
    ...
    url = f"{settings.REMOTE_MODEL_BASE_PATH}/other/embed_texts"
...
def classify():
    ...
    subclass_embeddings = {
        subclass: embed_for_subclass(subclass.name) for subclass in subclasses
    }
    ...
    for i, paper in tqdm(enumerate(papers)):
        ...
        for subclass, subclass_embedding in subclass_embeddings.items():
            title_similarity = (
                torch.tensor(subclass_embedding)
                .view(1, -1)
                .mm(torch.tensor(title_embeddings[i]).view(-1, 1))
                .flatten()[0]
            )
            abstract_similarity = (
                torch.tensor(subclass_embedding)
                .view(1, -1)
                .mm(torch.tensor(abstract_embeddings[i]).view(-1, 1))
                .flatten()[0]
            )
...
if __name__ == "__main__":
    ...
    # create_labels()
    # classify()
    dict1 = dict({"a": {"b": 100}, "y": 200})
    dict2 = dict({"a": {"c": 200}, "z": 400})
    dict1.update(dict2)
    print(dict1)
